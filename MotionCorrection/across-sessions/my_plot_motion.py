...
def my_plot_motion(OUT_PATH, path_csv1):
    ...
    axes[1].plot(np.degrees(motion['Rx']), label='Rx')
    axes[1].plot(np.degrees(motion['Ry']), label='Ry')
    axes[1].plot(np.degrees(motion['Rz']), label='Rz')
    axes[1].set_ylabel('Rotation [degree]')
    axes[1].set_xlabel('Volumes')
    axes[1].legend(loc="upper right")

    axes[2].plot(motion['FD'], label='FD (mean: {:.2f})'.format(np.mean(motion['FD'])))
    axes[2].axhline(y=0.5, xmin=0, xmax=3, c="red", linewidth=0.5, zorder=0)
